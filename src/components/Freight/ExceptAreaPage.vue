...
<script>
import {ExceptArea, ExceptAreaDelete} from "@/api/shipper";
  ...
  methods: {
    ...
    handleRowDelete(index, row) {
      this.$confirm('确定要删除?', '提示', {
        ...
        this.$request.post(ExceptAreaDelete + '?id=' + row.id).then((response) => {
    ...
    getInfo() {
      let that = this;
      this.$request.get(ExceptArea).then((response) => {
