...
import {AddExceptArea, ExceptAreaDetail, GetAreaData, SaveExceptArea} from "@/api/shipper";
...
export default {
  ...
  methods: {
    ...
    onSaveTemplate() {
      this.$refs['infoForm'].validate((valid) => {
        if (valid) {
          ...
          let that = this;
          this.$request.post(SaveExceptArea, {
    ...
    onAddTemplate() {
      ...
      this.$refs['infoForm'].validate((valid) => {
        if (valid) {
          ...
          let that = this;
          this.$request.post(AddExceptArea, {
            table: that.tableData,
            info: that.infoForm
          }).then((response) => {
            this.$message({
    ...
    getInfo() {
      ...
      //加载快递公司详情
      let that = this
      this.$request.post(ExceptAreaDetail + '?id=' + that.infoForm.id).then((response) => {
